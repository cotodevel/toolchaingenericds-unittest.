...
#define OP_ASSERT_EQUAL_INT(expected, actual) \
  ...
  if((int)(expected) != (int)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected %d != %d", __BASE_FILE__, __LINE__, (expected), (actual)); \
...
do { \
  if((unsigned int)(expected) != (unsigned int)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected %u != %u", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message); \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
  if((short)(expected) != (short)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected %hi != %hi", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message); \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
  if((unsigned short)(expected) != (unsigned short)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected %hu != %hu", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message); \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
  if((long)(expected) != (long)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected %ld != %ld", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message); \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
  if((unsigned long)(expected) != (unsigned long)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected %lu != %lu", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message); \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
  if((expected) != (actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected %f != %f", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++;                                                \
    opmock_print_error_messages();                                      \
    return;								\
...
do { \
  if(strcmp((expected), (actual)) != 0) {					\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected '%s' != '%s'", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++; \
    opmock_print_error_messages();								\
    return;								\
...
do { \
  if(!((expected))) {							\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d", __BASE_FILE__, __LINE__); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++; \
    opmock_print_error_messages(); 								\
    return;								\
...
do { \
  if((char)(expected) != (char)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected '%c' != '%c'", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
  if((unsigned char)(expected) != (unsigned char)(actual)) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, expected '%cu' != '%cu'", __BASE_FILE__, __LINE__, (expected), (actual)); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
    ...
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, buffers are not equal", __BASE_FILE__, __LINE__); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
    if(cmp_buffer((char *) (expected), (char *) (actual), (length)) == 0) {						\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:ASSERT failed. File %s-line %d, buffers are equal", __BASE_FILE__, __LINE__); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++; \
    opmock_print_error_messages();						\
    return;								\
...
do { \
  ...
  if (opmock_get_number_of_errors() != 0) {					\
    snprintf(op_error_message, OP_MESSAGE_LENGTH, "ERROR:Verify failed. File %s-line %d", __BASE_FILE__, __LINE__); \
    opmock_add_error_message((char *) op_error_message);                \
    opmock_test_error++;						\
    opmock_print_error_messages();						\
    return;								\
